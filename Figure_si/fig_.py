...
#	lines += ax.plot(t2, protocols[i]/20, '-', color=(al_f[i], 0, (1 - al_f[i])), lw=lw1)
...
lc = multiline(xs, ys, al_f, ax2, cmap='rainbow', lw=lw1)

axcb = fig.colorbar(lc)
axcb.set_label(r'$\lambda$', labelpad=0, y=0.5, size=ss)
...
ax2.text(0.72*xxx, yy + 0.04*(yyy - yy), r'$t/\tau$', size=ss)
ax2.text(xx  + 0.01*(xxx-xx), yyy - 0.1*(yyy - yy), r'$K^*_0(t)$', size=0.7*ss)
ax2.text(xxx - 0.1*(xxx-xx), yyy - 0.1*(yyy - yy), r'(c)', size=ssl)
...
lines += ax.plot(phase3[0][:-1], phase3[1][:-1], '-', marker='^', markersize=0.75, color=col3, lw=lw1, label=r'q = 0.79')
lines += ax.plot(phase4[0][:-1], phase4[1][:-1], '-', marker='.', markersize=1.5, color='green', lw=lw1, label=r'q = 1.57')
ax.labels = [l.get_label() for l in lines]
ax.legend(lines, ax.labels, bbox_to_anchor=(0.55, 0.8), fontsize=4.5, frameon=False)

ax.text(0.8*xxx, yy + 0.2, r'$\lambda$', size=ss)
ax.text(xx + 0.05, 0.85*yyy, r'$\mu$', size=ss)
ax.text(xxx - 0.1*(xxx-xx), yyy - 0.1*(yyy - yy), r'(b)', size=ssl)
